jQuery(document).ready(function($) {
	...
	$(document).on('click', '#gmw-yl-edit', function(){
    ...
    $(document).on('click', '.gmw-yl-tab', function(){
    ...
    $(document).on('click', '#gmw-yl-delete', function(){		
		delete_location();
	});
	
	//add class when click on update location button
    $(document).on('click', '#gmw-yl-get-latlng, #gmw-yl-get-address', function(){		
		$('#gmw-yl-update-location').addClass('update');
	});
	
    //locator button clicked 
    $(document).on('click', '#gmw-yl-locator-btn', function(){    	
    	$('#gmw-yl-get-latlng, #gmw-yl-get-address').removeClass('changed');
    	$("#gmw-yl-spinner").show();
  		getLocationBP();
  	}); 
	...
	$(document).on('click', '#gmw-yl-get-address', function(){ 	
		...
		for ( x in address ) {
			...
			if ( address[x].types == 'route' ) {
				...
				if ( street_number != false ) {
					...
					$("#_wcv_store_address1").val(street);
				} else {
					$("#gmw-street").val(street);
					$("#_wcv_store_address1").val(street);
				}
	...
	$(document).on('click', '#gmw-yl-get-latlng', function(){	
    ...
    function saveLocation() {
    	...
    	var usergetid = $('#gmw-yl-form #this-user-id').val();
    	    	
		$.ajax({
			type       	: "post",
			data  		: {action:'rh_gmw_fl_update_location', 'formValues': $('#gmw-yl-form').serialize(), 'usergetid' : usergetid },		
			url        	: ajaxurl,
    ...
    function delete_location() {
		...
		$.ajax({
			type       	: "post",
			data  		: {action:'rh_gmw_fl_delete_location', 'usergetid' : usergetid },		
			url        	: ajaxurl,
			success:function(data){
